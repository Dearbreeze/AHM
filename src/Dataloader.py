...
def PathList(path):
    ...
    train_path_list = os.listdir(path + '/Train')
    train_path_list.sort()
    val_path_list = os.listdir(path + '/Val')
    val_path_list.sort()
    test_path_list = os.listdir(path + '/Test')
    test_path_list.sort()
    for files in train_path_list:
        if os.path.splitext(files)[1] == '.jpg':
            train_jpg_list.append(files)
        else:
            train_png_list.append(files)

    for files in test_path_list:
        if os.path.splitext(files)[1] == '.jpg':
            test_jpg_list.append(files)
        else:
            test_png_list.append(files)

    for files in val_path_list:
        if os.path.splitext(files)[1] == '.jpg':
            val_jpg_list.append(files)
        else:
            val_png_list.append(files)
    # print(train_jpg_list[4],train_png_list[4])
    return train_jpg_list, train_png_list, test_jpg_list, test_png_list, val_jpg_list, val_png_list


class MakeDataset(Dataset):
    def __init__(self, baseroot, jpglist, pnglist, train, img_size=None):
    ...
    def __getitem__(self, index):
        ...
        targets = (pngimg / 255).astype(np.float32)
        ...
        file_name = os.path.splitext(self.jpglist[index])[0]
        # print(type(inputs))
